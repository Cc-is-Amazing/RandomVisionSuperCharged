...
internal static class RandomVisionSuperChargedI18n
{
    ...
    {
        TrySubscribe();
        var locManagerLanguage = TryGetLocManagerLanguage();
        if (!string.IsNullOrWhiteSpace(locManagerLanguage))
        {
            _language = NormalizeLanguageCode(locManagerLanguage);
            return;
        }
    ...
    private static string? TryGetLocManagerLanguage()
    {
        try
        {
    ...
    private static string NormalizeLanguageCode(string? language)
    {
        ...
        {
            "zh" or "zhs" or "zh_cn" or "zh_hans" or "zh_sg" => "zhs",
            "en" or "eng" or "en_us" or "en_gb" => "en",
            _ => normalized
        };
    }
}
