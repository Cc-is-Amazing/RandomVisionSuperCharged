# RandomVision Agent Handoff

## Workspace

- Current working folder: `/Users/cynthialiu/Desktop/RandomVision`
...
RandomVision is a Godot/C# Slay the Spire 2 mod that previews event outcomes, map encounter queues, draw pile order, and related seeded predictions.

The user expects every complete fix to be packaged into:

- `/Users/cynthialiu/Desktop/RandomVision/RandomVisionBuild/RandomVision.dll`
- `/Users/cynthialiu/Desktop/RandomVision/RandomVisionBuild/RandomVision.json`
- `/Users/cynthialiu/Desktop/RandomVision/RandomVisionBuild/RandomVision.pck`
...
```sh
dotnet build RandomVision.csproj -c Release \
...
```sh
dotnet publish RandomVision.csproj -c Release \
  -p:Sts2DataDir="/Users/cynthialiu/Library/Application Support/Steam/steamapps/common/Slay the Spire 2/SlayTheSpire2.app/Contents/Resources/data_sts2_macos_arm64" \
  -p:Sts2Path="/Users/cynthialiu/Library/Application Support/Steam/steamapps/common/Slay the Spire 2" \
  -p:ModOutputDir="/Users/cynthialiu/Desktop/RandomVision/RandomVisionBuild" \
...
```sh
chflags nohidden RandomVisionBuild/RandomVision.dll
ls -laO@ RandomVisionBuild
shasum -a 256 RandomVisionBuild/RandomVision.dll RandomVisionBuild/RandomVision.json RandomVisionBuild/RandomVision.pck
dotnet list RandomVision.csproj package --vulnerable --include-transitive
...
- Event overlay: `Services/RandomVisionEventOverlay.cs`
- Event prediction registry: `Services/RandomVisionPreviewRegistry.cs`
- Map overlay: `Services/RandomVisionMapEncounterOverlay.cs`
...
Primary recent work is in `Services/RandomVisionMapEncounterOverlay.cs`.
...
- Replaced game `NHoverTipSet` for map rows with a RandomVision-owned tooltip node named `RandomVisionMapEncounterTooltip`.
- Tooltip is added under `RandomVisionMapEncounterRoot` with high `ZIndex`, so it should render above the overlay panel rather than under it.
...
```text
System.NullReferenceException at RandomVision.Services.RandomVisionMapEncounterOverlay.ReadAncientOptionTitles(AncientEventModel ancient)
```
...
The user has asked for many event/Ancient-specific prediction fixes in `RandomVisionPreviewRegistry.cs`. Important implemented cases include:
...
- Draw-pile order functionality was moved into RandomVision through `Patches/OrderedDrawPilePatch.cs`.
...
- The current map overlay is not the old Math focus patch. It uses the same RandomVision overlay style instead.
...
```text
143cbe044991f18b59c7642fd53611a870bf8ba34895ce9b88de68ae22aaae16  RandomVisionBuild/RandomVision.dll
0354606788c49950ce6ba29cfd6f61b3ac5ed65a8c14a487f6dc6989060138cb  RandomVisionBuild/RandomVision.json
71977df9b4b3ef676d9e35ab663a7e2c34cb45cdfac366586075130af8fc0123  RandomVisionBuild/RandomVision.pck
```

Build and publish passed. `dotnet list RandomVision.csproj package --vulnerable --include-transitive` reported no vulnerable packages.
